...
> **Note:** Not all databases support every filter code. Findpapers validates the query against each database's capabilities and skips databases that cannot handle the query. See [Search Databases](search-databases.md) for per-database support.
