...
class TestSemanticScholarSortParam:
    """Tests that the sort parameter is only sent when a date range is present."""

    def test_sort_present_with_date_range(
        self, simple_query, semantic_scholar_sample_json, mock_response
    ):
        """sort=publicationDate:desc is included when since/until are given."""
        searcher = SemanticScholarConnector()
        first_page = mock_response(json_data=semantic_scholar_sample_json)
        first_page.raise_for_status = MagicMock()
        second_page = mock_response(json_data={"total": 0, "token": None, "data": []})
    ...
    def test_sort_absent_without_date_range(
        self, simple_query, semantic_scholar_sample_json, mock_response
    ):
        """sort is omitted when no date range is specified (avoids S2 500 error)."""
        searcher = SemanticScholarConnector()
        first_page = mock_response(json_data=semantic_scholar_sample_json)
        first_page.raise_for_status = MagicMock()
        second_page = mock_response(json_data={"total": 0, "token": None, "data": []})
        ...
        with patch.object(searcher, "_rate_limit"):
            searcher.search(simple_query, max_papers=5)
